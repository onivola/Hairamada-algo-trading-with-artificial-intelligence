...
input double w20=0.5;
input double w21=0.5;
input double w22=0.5;
input double w23=0.5;
input double w24=0.5;
input double w25=0.5;
input double w26=0.5;
input double w27=0.5;
input double w28=0.5;
input double w29=0.5;
...
double            inputs[30];   // array for storing inputs
double            weight[30];   // array for storing weights
...
//|                                                                  |
//+------------------------------------------------------------------+
void GetMA(int ma_period,int decalage,double &myPriceArray[]) {
   
   int signal;
   int iACDefinition = iMA(_Symbol,my_timeframe,ma_period,decalage,MODE_EMA,PRICE_CLOSE);
   ArraySetAsSeries(myPriceArray,true);
   CopyBuffer(iACDefinition,0,0,10,myPriceArray); 
}
int OnInit()
  {
   GetMA(55,5,iMA55_buf);
   GetMA(5,5,iMA5_buf);
...
//--- In the loop, fill in the array of inputs with the pre-normalized indicator values
   ...
   int i = 0;
      while(i<10)
     {
      inputs[j]=(((iRSI_buf[i]-x_min)*(d2-d1))/(x_max-x_min))+d1;
      i = i + 1;
      j = j+1;
     }
     i = 0;
    ...
    while(i<10)
     {
      inputs[j]=(((iMA55_buf[i]-x_min)*(d2-d1))/(x_max-x_min))+d1;
      i = i + 1;
      j = j+1;
     }
     ...
     i=0;
      while(i<10)
     {
      inputs[j]=(((iMA5_buf[i]-x_min)*(d2-d1))/(x_max-x_min))+d1;
      i = i + 1;
      j = j+1;
     }
