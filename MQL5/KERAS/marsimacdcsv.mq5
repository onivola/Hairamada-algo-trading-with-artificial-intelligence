...
input datetime           date_finish=D'2022.02.01 00:00'; 
...
double MACDHandle(ENUM_TIMEFRAMES period) {

   int iACDefinition = iMACD(_Symbol,period,8,10,8,PRICE_LOW);
   if(iACDefinition==INVALID_HANDLE)
   {
...
//--- receive MACD indicator handle
   ...
   int ma_handle1 = IMAHandle(PERIOD_M1,1,MODE_LWMA);
   int macd_handle = MACDHandle(PERIOD_M1);
   ResetLastError();
   if(CopyBuffer(ma_handle5,0,InpDateStart,date_finish,ma_buff5)==-1)
      ...
      if(CopyBuffer(ma_handle12,0,InpDateStart,date_finish,ma_buff12)==-1)
     {
      ...
      ResetLastError();
      if(CopyBuffer(ma_handle1,0,InpDateStart,date_finish,ma_buff1)==-1)
     {
      PrintFormat("Failed to copy indicator values. Error code = %d",GetLastError());
      return;
     }
