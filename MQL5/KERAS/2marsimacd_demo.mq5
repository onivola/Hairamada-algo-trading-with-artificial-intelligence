...
float iMA(int n,ENUM_TIMEFRAMES periode,int ma_period,ENUM_MA_METHOD ma_method) {
   ...
   CopyBuffer(iACDefinition,0,0,100,myPriceArray); 
...
double GetiMACD(ENUM_TIMEFRAMES period, int n) {
   ...
   CopyBuffer(iACDefinition,0,0,100,macd);
   return macd[n];
}
...
//---double ma1_5 = iMA(1,PERIOD,5,MODE_LWMA);
      double ma1_12 = iMA(1,PERIOD,12,MODE_LWMA);
       ...
       double rsi = GetRSI(0);
       double macd = GetiMACD(PERIOD,0); 
//---
   return(INIT_SUCCEEDED);
  }
void OnTick()
  {
     double ema01 = iMA(0,PERIOD,1,MODE_LWMA);
      ...
      if(ma_5 >= ma_12 && (ma1_5 <= ma1_12 || ma2_5 <= ma2_12) && pas==false) {
         ...
         BuildDataset(ma_buff1,ma_buff5,ma_buff12,rsi_buff5,macd_buff,60);
         
            //BuyPosition();
          
            //nb_pos = nb_pos+1;
          while(1) {
               bool datasave = SaveDataset(ma_buff1,ma_buff5,ma_buff12,rsi_buff5,macd_buff);
       ...
       } else if(ma_5 > ma_12 && (ma1_5 > ma1_12 && ma2_5 > ma2_12) && pas==true && posTotal==0) {
            pas= false;
            Print("false-----------------------------------------");
       }
...
bool BuildDataset(double& ma_buff1[], double& ma_buff5[],double& ma_buff12[],double& rsi_buff5[],double& macd_buff[],double size) {
   ArrayResize(ma_buff5,size);
   ArrayResize(ma_buff12,size);
    ArrayResize(rsi_buff5,size);
     ArrayResize(ma_buff1,size);
      ArrayResize(macd_buff,size);
   //Print(ArraySize(ma_buff12));
   
      for(int i=0;i<size;i++) {
ma_buff1[i] =  iMA(i,PERIOD,1,MODE_LWMA);
   ma_buff5[i] = iMA(i,PERIOD,5,MODE_LWMA);
   ma_buff12[i] = iMA(i,PERIOD,12,MODE_LWMA);
   rsi_buff5[i] = GetRSI(i);
   macd_buff[i] =  GetiMACD(PERIOD,i);
   Print(GetRSI(i));
      }
      return true;
...
bool SaveDataset(double& ma_buff1[],double& ma_buff5[],double& ma_buff12[],double& rsi_buff5[],double& macd_buff[]) {
   ...
   if(file_handle!=INVALID_HANDLE)
     {
      ...
      FileWrite(file_handle,"i,MA1,MA5,MA12,RSI,MACD");
      //--- write the time and values of signals to the file
      for(int i=0;i<ArraySize(ma_buff12);i++)
         FileWrite(file_handle,i+","+ma_buff1[i]+","+ma_buff5[i]+","+ma_buff12[i]+","+rsi_buff5[i]+","+macd_buff[i]);
      //--- close the file
