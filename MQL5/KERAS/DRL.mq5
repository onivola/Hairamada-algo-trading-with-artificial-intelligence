       ...
       if(saved==true  && second>2) {
         ...
         if(predict!=last_predict && predict!=-1) {
            ...
            if(predict>=1 && predict<10000000) {
               if(posTotal<=1  && (isbuy==0 || isbuy==2 || isbuy==1)) {
                  CloseAll(_Symbol);
                  BuyPosition();
               }
            ...
            if(predict<=-1) {
               if(posTotal<=1  && (isbuy==1 || isbuy==2 || isbuy==0)) {
                  CloseAll(_Symbol);
                  SellPosition();
   ...
   if(file_handle!=INVALID_HANDLE)
     {
      ...
      for(int i=203;i>=0;i--) {
         ...
         FileWrite(file_handle,j+","+result[2]+","+result[1]+","+result[0]+","+result[3]+","+result[4]);
         j = j+1;
      }
      //--- close the file
