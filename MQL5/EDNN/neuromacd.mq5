...
// Inputvector2[] - Array of ann input data
double Inputvector2[];
...
double
ann_run(int ann, double &vector22[])
{
    int ret;
    double out;
    ret = f2M_run(ann, vector22);
    if(ret < 0) {
...
int
anns_run_parallel(int anns_count, int &anns[], double &input_vector2[])
{
    int ret;

    ret = f2M_run_parallel(anns_count, anns, input_vector2);
...
ann_prepare_input()
{
	...
	Inputvector2[i + 1] =
	    10 * iMACD(NULL, 0, FastMA, SlowMA, SignalMA, PRICE_CLOSE,
			MODE_SIGNAL, i * 3);
	Inputvector2[i + 2] = Inputvector2[i - 2] - Inputvector2[i - 1];
    }
}
...
run_anns()
{
    ...
    if(Parallel) {
	anns_run_parallel(AnnsNumber, AnnsArray, Inputvector2);
    }
	...
	if(Parallel) {
	    ...
	    AnnOutputs[i] = ann_run(AnnsArray[i], Inputvector2);
	}
    }
}

void
ann_train(int ann, double &input_vector2[], double &output_vector2[])
{
    if(f2M_train(ann, input_vector2, output_vector2) == -1) {
...
init()
{
    ...
    ArrayResize(Inputvector2, AnnInputs);
