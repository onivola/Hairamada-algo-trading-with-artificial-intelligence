...
double
ann_run (int ann, double &vector2[])
{
    int ret;
    double out;
    ret = f2M_run (ann, vector2);
    if (ret < 0) {
...
ann_prepare_input ()
{
	...
	InputVector[i] =
	    10 * iMACD(_Symbol,PERIOD_M1, FastMA, SlowMA, SignalMA, PRICE_CLOSE,i * 3);
	InputVector[i + 1] =
	    10 * iMACD (_Symbol,PERIOD_M1,  FastMA, SlowMA, SignalMA, PRICE_CLOSE,
			MODE_CLOSE, i * 3);
