...
DeepNeuralNetwork dnnf(numInput,numHiddenA,numHiddenB,numOutput);

//--- weight values
//les valeurs de weight
input double w0=1.0;
input double w1=1.0;
input double w2=1.0;
input double w3=1.0;
...
//calculer le pourcentage de l'ombre superieure ,de l'ombre inferieure et du corps dans la base de somme 100%
//compute the percent of the upper shadow, lower shadow and body in base of sum 100%
   ...
   dnnf.SetWeights(weight);//dans la bibliotheque deepneronalnetworkfanoavana.mql

   double yValues[];
   dnnf.ComputeOutputs(_xValues,yValues);//dans la bibliotheque deepneronalnetworkfanoavana.mql
   
//---si la valeur de sortie de neurone est superieur a 60 %
//--- if the output value of the neuron is mare than 60%
   if(yValues[0]>0.6)
     {
     //verifie s'il y a une position ouverte
...
//--- si la valeur de sortie du neurone est superieur a 60%
//--- if the output value of the neuron is mare than 60%
   if(yValues[1]>0.6)
     {
   ...
   if(yValues[2]>0.6)
     {
     //fermer n'inporte quelle position 
...
//|pourcentage de chaque partie de la bougie par rapport a la taille totale     |
//+------------------------------------------------------------------+
int CandlePatterns(double high,double low,double open,double close,double uod,double &xInputs[])
  {
   double p100=high-low;//taille totale de la bougie 
   double highPer=0;
   double lowPer=0;
   double bodyPer=0;
   double trend=0;
