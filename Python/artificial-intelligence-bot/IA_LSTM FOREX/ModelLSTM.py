...
class ModelLSTM:
    ...
    def DFdataset_1(self,SYMBOL,TIMEFRAME,sy,sm,sd,ey,em,ed):
        ...
        df['time'] = pd.to_datetime(df['time'], unit='s')
        return df
    def DFdataset_current(self,SYMBOL,TIMEFRAME,nb):
        
        # request ohlc data a save them in a pandas DataFrame
        #bars = mt5.copy_rates_range(SYMBOL, TIMEFRAME, start_dt, end_dt)
        
        bars = mt5.copy_rates_from_pos(SYMBOL, TIMEFRAME, 0, nb)
        print(bars)
        df = pd.DataFrame(bars)[['time', 'open', 'high', 'low', 'close']]
        df['time'] = pd.to_datetime(df['time'], unit='s')
        return df
    def DFdataset_current2(self,SYMBOL,TIMEFRAME,nb):
    ...
    def Iloc_df(self,dataset_train):
        training_set = dataset_train.iloc[:, 4:5].values #open values
        #dataset_train.head()
        return training_set
    ...
    def get_inputs2(self,dataset_train,dataset_test):
        dataset_total = pd.concat((dataset_train['close'], dataset_test['close']), axis = 0)
    ...
    def plot_predict2(self,real_stock_price,predicted_stock_price):
        ...
        plt.plot(predicted_stock_price, color = 'black', label = 'Predicted TATA Stock Price')
        plt.title('TATA Stock Price Prediction')
        plt.xlabel('Time')
        plt.ylabel('TATA Stock Price')
        plt.legend()
    ...
    def get_x(self,df):
        ...
        for i in range(0,len(training)):
            training_set.append([training['close'][i]])
        #print(training_set)
