...
class Model:
    ...
    def PlotFitModel(self,history):
        ...
        plt.ylabel('Acc')
        plt.show();
    def fitModel_regretion(self,name,epoch,x_train,y_train,x_test,y_test,lr):

        # Build the neural network
        model = Sequential()
        model.add(Dense(160, input_dim=x_train.shape[1], activation='relu')) # Hidden 1
        model.add(Dense(320, activation='relu')) # Hidden 2
        model.add(Dense(2, activation='relu')) # Hidden 2
    ...
    def test_validation(self,model,X,Y,nb_x):
        ...
        for i in range(0,nb_x):
            ...
            print(Yt[i])
            yt = float(Yt[i])
            if(yt>0.5 and res>0.5):
                gainb = gainb+1
            elif(yt<0.5 and res<0.3):
                gains = gains+1
            elif(yt<0.5 and res>0.5):
                perteb = perteb+1
            elif(yt>0.5 and res<0.3):
                pertes = pertes+1
