...
class Dataset:
    ...
    def reshape(self,P):
        ...
        for i in range(len(P)):
            for j in range(len(P[i])):
                Pn.append(P[i][j])
        return Pn
    def get_obs_predict(self,df2,i,b1,whole,whole_vol):
        X=[]
        Y=[]
        Px=[]
        Py=[]
        ...
        for x in range(i,i+b1):
            Px.append(self.vectorisation(df2['open'][x],df2['high'][x],df2['low'][x],df2['close'][x],False,False,whole,whole_vol))
            close1 = df2['close'][x]
        ...
        XY.append([0])
        return XY 
    def get_obs(self,df2,i,b1,b2,whole,whole_vol): #b1=5 b2 = 9
        X=[]
        Y=[]
        Px=[]
        Py=[]
        ...
        close = 0
        for x in range(i,i+b1):
            Px.append(self.vectorisation(df2['open'][x],df2['high'][x],df2['low'][x],df2['close'][x],df2['tick_volume'][x],False,whole,whole_vol))
            close1 = df2['close'][x]
        
        for y in range(i+b1,i+b1+b2):
            Py.append(self.vectorisation(df2['open'][y],df2['high'][y],df2['low'][y],df2['close'][y],df2['tick_volume'][y],False,whole,whole_vol))
            close = df2['close'][y]
        Y.append(close1)
        Y.append(Py)
        Y.append(close)
    ...
    def vectorisationOHOL(self,Open,High,Low,Close):
        ...
        return part
    def vectorisation(self,Open,High,Low,Close,Volume,troi,whole,whole_vol):
        #whole=3500
        part=[0,0,0,0,0]
        if(troi):
            part=[0,0,0,0,0,0]
