...
df1 = DFdataset_1(SYMBOL,TIMEFRAME,2022,5,5,2022,5,7)
...
#model.save('pp')
modelBuy = PPO2.load("buy")
modelSell = PPO2.load("sell")
#-6497.269171764005
#-28.450912496060482
#-23.133360367120986
def plotxy(list_reward):
...
while(1):
    ...
    if 'open' in dc:
        ...
        if(new_nb!=last_nb):
            ...
            obs = np.array(xy[0][0])
            actionB, _statesB = modelBuy.predict(obs)
            actionS, _statesS = modelSell.predict(obs)
            print("Action Buy-------------")
            print(actionB)
            print("Action Sell-------------")
            print(actionB)
            if(actionB==1 and actionS=0):
                buy = buy+1
                pos = buy
            elif(actionB==0 and actionS=1):
                sell = sell-1
                pos = sell
            else:
                hold = hold+1
