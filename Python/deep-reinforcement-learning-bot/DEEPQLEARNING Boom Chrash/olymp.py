...
env1 = DummyVecEnv([lambda: StockTradingEnv1ch(df1)])
env2 = DummyVecEnv([lambda: StockTradingEnv1ch(df2)])
...
while(1):
    ...
    #SYMBOL = "Crash 500 Index"
    TIMEFRAME = mt5.TIMEFRAME_M1
    df2 = DFdatasetCurrent(SYMBOL,TIMEFRAME,1,15)

    if(CheckPriceChange(df2,last_time)):
        print("++++++predict+++++")
        last_time = str(df2['time'][len(df2)-1])
        xy = dataset.get_obs_predict(df2,0,14)
        obs = np.array(xy[0][0])
