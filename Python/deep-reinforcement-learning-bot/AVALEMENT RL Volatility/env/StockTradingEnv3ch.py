...
class StockTradingEnv3ch(gym.Env):
    ...
    def __init__(self, df):
        super(StockTradingEnv3ch).__init__()
        self.nb_obs = 0
        self.df = df
        self.end = 0
        ...
        self.observation_space = spaces.Box(
            low=0, high=1, shape=(200, 4), dtype=np.float16)
    ...
    def _next_observation(self):
        ...
        xy = self.dataset.get_obs(self.df,self.current_step,200,1,self.whole)
    ...
    def step(self, action):
        ...
        info = self.end
        self.end = 0
        if(self.current_step > (len(self.df.loc[:, 'open'].values) - 212)):
            #done = True
            print("reward=")
            print(self.reward)
            print("gain=")
