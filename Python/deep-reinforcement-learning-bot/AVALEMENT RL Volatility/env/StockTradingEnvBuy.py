...
class StockTradingEnvBuy(gym.Env):
    """A stock trading environment for OpenAI gym"""
    metadata = {'render.modes': ['human']}

    def __init__(self, df):
        super(StockTradingEnvBuy).__init__()
        self.nb_obs = 0
        self.df = df
        self.predict = 13
        ...
        self.observation_space = spaces.Box(
            low=0, high=1, shape=(204, 4), dtype=np.float16)
    ...
    def _next_observation(self):
        ...
        xy = self.dataset.get_obs(self.df,self.current_step,204,self.predict,self.whole)
    ...
    def get_reward(self,y,action_type):
        ...
        if(action_type ==0): #Sell
            for i in range(0,self.predict):
                ch = y[1][i][1]
                ch_val = y[1][i][3]
                if(ch_val==0):
                    reward1 = reward1+self.GetGain(ch)
                if(ch_val==1):
                    reward1 = reward1-self.GetGain(ch)-0.02
            #self.current_step += 1
    ...
    def step(self, action):
        ...
        #if(self.nb_obs ==1):
        #print(self.current_step)
        #if(self.reward>=0 and istest==False):
        self.current_step += self.predict
        #if(istest==True):
            #self.current_step += 1
        obs = self._next_observation()
        #self.nb_obs =0 
        ...
        info = self.end
        self.end = 0
        if(self.current_step > (len(self.df.loc[:, 'open'].values) - (204+self.predict+self.predict))):
            #done = True
            print("reward=")
            print(self.reward)
            print("gain=")
